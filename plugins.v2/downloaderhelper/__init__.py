...
from app.helper.downloader import DownloaderHelper as SystemDownloaderHelper
from app.helper.sites import SitesHelper
from app.log import logger
from app.modules.qbittorrent.qbittorrent import Qbittorrent
from app.modules.transmission.transmission import Transmission
from app.plugins import _PluginBase
from app.plugins.downloaderhelper.module import TaskContext, TaskResult, TorrentField, TorrentFieldMap, DownloaderTransferInfo, EventDeleteTorrentStrategy
from app.schemas import NotificationType, DownloaderConf, ServiceInfo
...
class DownloaderHelper(_PluginBase):
    ...
    # 常量
    # 插件说明链接
    __help_url = 'https://github.com/hotlcc/MoviePilot-Plugins-Third/tree/main/plugins.v2/downloaderhelper'
    # 多级根域名，用于在打标时做特殊处理
    ...
    # 默认值替身
    __default_value_standing = "__default__"
    # 仪表板组件key前缀
    # 活动种子组件
    __dashboard_widget_key_prefix_active_torrent = "active_torrent_"
    # 实时速率组件
    __dashboard_widget_key_prefix_speed = "speed_"
    ...
    # 系统下载器服务帮助类
    __downloader_helper = SystemDownloaderHelper()
    ...
    __config_default: Dict[str, Any] = {
        ...
        'dashboard_widget_target_downloaders': [__default_value_standing],
        ...
        'dashboard_speed_widget_target_downloaders': [__default_value_standing],
        'event_delete_torrent_strategy': EventDeleteTorrentStrategy.DELAYED.name,
    }
    # 插件用户配置
    ...
    def init_plugin(self, config: dict = None):
        ...
        logger.debug(f"插件配置加载完成：{config}")
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        # 事件删种策略选项
        event_delete_torrent_strategy_options = [{
            'title': strategy.name_,
            'value': strategy.name
        } for strategy in EventDeleteTorrentStrategy if strategy]
        # 事件删种策略表单hint
        event_delete_torrent_strategy_hint = "；".join([f"{strategy.name_}：{strategy.desc}" for strategy in EventDeleteTorrentStrategy if strategy]) + "。"
        # 非全选标签 默认值
        not_select_all_tag_default = self.__config_default.get("not_select_all_tag")
        # 站点标签前缀 默认值
        tag_prefix_default = self.__config_default.get("tag_prefix")
        # 全部下载器配置
        downloader_configs = self.__get_downloader_configs(include_disabled=True)
        # 下载器下拉选项
        downloader_select_options = [{
            'title': '[系统默认]',
            'value': self.__default_value_standing
        }]
        if downloader_configs:
            for downloader_name in downloader_configs.keys():
                if not downloader_name:
                    continue
                downloader_select_options.append({
                    'title': downloader_name,
        ...
        elements = [{
            ...
            'content': [{  # 业务无关总控
                ...
                'content': [{
                    ...
                    'content': [{
                        ...
                        'props': {
                            ...
                            'label': '非全选标签',
                            'placeholder': not_select_all_tag_default,
                            'hint': f'种子未全选文件时添加的标签，默认值为“{not_select_all_tag_default}”'
                        }
                    }]
                }, {
                    'component': 'VCol',
                    ...
                    'content': [{
                        ...
                        'props': {
                            'model': 'tag_prefix',
                            'label': '站点标签前缀',
                            'placeholder': tag_prefix_default,
                            'hint': f'给种子添加站点标签时的标签前缀，默认值为“{tag_prefix_default}”'
                        }
                    }]
                }, {
                    'component': 'VCol',
                    ...
                    'content': [{
                        'component': 'VSelect',
                        'props': {
                            'model': 'event_delete_torrent_strategy',
                            'label': '事件删种策略',
                            'items': event_delete_torrent_strategy_options,
                            'hint': f'事件触发删种时以何种策略删种，缺省时为【延迟删种】。{event_delete_torrent_strategy_hint}'
                        }
                    }]
                }]
            }, {
                        ...
                        'content': [{
                            ...
                            'content': [{
                                ...
                                'props': {
                                    ...
                                    'multiple': True,
                                    'chips': True,
                                    'items': downloader_select_options,
                                    'hint': '选择要展示的目标下载器。'
                                }
                            }]
                        }, {
                ...
                'content': [{
                    ...
                    'content': self.__build_downloader_tabs_window_form_elements()
                }]
            }, {
                'component': 'VRow',
                'content': [{
    ...
    def __build_downloader_tabs_window_form_elements(self) -> List[dict]:
        """
        构造下载器VTabls+VWindow表单组件
        """
        # 全部下载器配置
        downloader_configs = self.__get_downloader_configs(include_disabled=True)
        return [{
            ...
            'content': [self.__build_downloader_tabs_form_element(downloader_configs=downloader_configs)]
        }, {
            'component': 'VWindow',
            'props': {
                'model': '_tabs'
            },
            'content': [self.__build_downloader_window_form_element(downloader_configs=downloader_configs)]
        }] if downloader_configs else [{
            'component': 'div',
            'props': {
                'style': {
                    'color': 'red',
            ...
            'text': '您尚未配置任何下载器，请先配置！'
        }]

    @classmethod
    def __build_downloader_tabs_form_element(cls, downloader_configs: Dict[str, DownloaderConf]) -> dict:
        """
        构造下载器VTabs表单组件
        """
        return {
            ...
            'content': cls.__build_downloader_tab_form_elements(downloader_configs=downloader_configs)
        } if downloader_configs else {}

    @classmethod
    def __build_downloader_tab_form_elements(cls, downloader_configs: Dict[str, DownloaderConf]) -> List[dict]:
        """
        构造下载器VTab表单组件
        """
        return [{
            ...
            'text': downloader_name
        } for downloader_name in downloader_configs.keys() if downloader_name] if downloader_configs else []

    @classmethod
    def __build_downloader_window_form_element(cls, downloader_configs: Dict[str, DownloaderConf]) -> dict:
        """
        构造下载器VWindow表单组件
        """
        return {
            ...
            'content': cls.__build_downloader_window_item_form_elements(downloader_configs=downloader_configs)
        } if downloader_configs else {}

    @classmethod
    def __build_downloader_window_item_form_elements(cls, downloader_configs: Dict[str, DownloaderConf]) -> List[dict]:
        """
        构造下载器VTab表单组件
        """
        return [{
            ...
            'content': cls.__build_downloader_window_item_content_form_elements(model_prefix=downloader_name)
        } for downloader_name in downloader_configs.keys() if downloader_name] if downloader_configs else []

    @classmethod
    def __build_downloader_window_item_content_form_elements(cls, model_prefix: str) -> List[dict]:
        """
        构造下载器VTab内容表单组件
        """
        return [{
            ...
            'content': [{
                ...
                'content': [{
                    'component': 'VSwitch',
                    'props': {
                        'model': f'{model_prefix}_enable_tagging',
                        'label': '自动标签',
                ...
                'content': [{
                    'component': 'VSwitch',
                    'props': {
                        'model': f'{model_prefix}_enable_seeding',
                        'label': '自动做种',
                ...
                'content': [{
                    'component': 'VSwitch',
                    'props': {
                        'model': f'{model_prefix}_enable_delete',
                        'label': '自动删种',
    ...
    def get_dashboard_meta(self) -> Optional[List[Dict[str, str]]]:
        ...
        if not enable_dashboard_active_torrent_widget and not enable_dashboard_speed_widget:
            return dashboard_meta
        # 所有有效的下载器服务信息
        downloader_services = self.__get_downloader_services()
        # 活动种子
        if enable_dashboard_active_torrent_widget:
            downloader_names = self.__get_dashboard_active_torrent_widget_target_downloader_names()
            for downloader_name in downloader_names:
                if downloader_services.get(downloader_name):
                    dashboard_meta.append({
                        "key": f"{self.__dashboard_widget_key_prefix_active_torrent}{downloader_name}",
                        "name": f"活动种子 #{downloader_name}",
                    })
        # 实时速率
        if enable_dashboard_speed_widget:
            downloader_names = self.__get_dashboard_speed_widget_target_downloader_names()
            for downloader_name in downloader_names:
                if downloader_services.get(downloader_name):
                    dashboard_meta.append({
                        "key": f"{self.__dashboard_widget_key_prefix_speed}{downloader_name}",
                        "name": f"实时速率 #{downloader_name}",
                    })
        return dashboard_meta
    ...
    def get_dashboard(self, key: str = None, **kwargs) -> Optional[Tuple[Dict[str, Any], Dict[str, Any], List[dict]]]:
        ...
        """
        if not key or not self.get_state():
            return None
        enable_dashboard_active_torrent_widget = self.__check_enable_dashboard_active_torrent_widget()
        enable_dashboard_speed_widget = self.__check_enable_dashboard_speed_widget()
        if not enable_dashboard_active_torrent_widget and not enable_dashboard_speed_widget:
            return None
        # 活动种子
        if enable_dashboard_active_torrent_widget and key.startswith(self.__dashboard_widget_key_prefix_active_torrent):
            downloader_name = key.removeprefix(self.__dashboard_widget_key_prefix_active_torrent)
            dashboard_active_torrent_widget_target_downloader_names = self.__get_dashboard_active_torrent_widget_target_downloader_names()
            if downloader_name in dashboard_active_torrent_widget_target_downloader_names:
                return self.__get_dashboard_active_torrent_widget(downloader_name=downloader_name)
        # 实时速率
        elif enable_dashboard_speed_widget and key.startswith(self.__dashboard_widget_key_prefix_speed):
            downloader_name = key.removeprefix(self.__dashboard_widget_key_prefix_speed)
            dashboard_speed_widget_target_downloader_names = self.__get_dashboard_speed_widget_target_downloader_names()
            if downloader_name in dashboard_speed_widget_target_downloader_names:
                return self.__get_dashboard_speed_widget(downloader_name=downloader_name)
        return None

    def __get_dashboard_active_torrent_widget(self, downloader_name: str) -> Optional[Tuple[Dict[str, Any], Dict[str, Any], List[dict]]]:
        """
        获取仪表板活动种子组件
        """
        if not downloader_name:
        ...
        # 全局配置
        attrs = {
            'title': f'活动种子 #{downloader_name}'
        }
        if self.__check_downloader_instance(instance=service_info.instance):
            attrs['refresh'] = self.__get_config_item('dashboard_widget_refresh')

        # 页面元素
        elements = self.__get_dashboard_active_torrent_widget_elememts(service_info=service_info)

        return cols, attrs, elements

    def __get_dashboard_speed_widget(self, downloader_name: str) -> Optional[Tuple[Dict[str, Any], Dict[str, Any], List[dict]]]:
        """
        获取仪表板实时速率组件
        """
        if not downloader_name:
        ...
        # 全局配置
        attrs = {
            'title': f'实时速率 #{downloader_name}'
        }
        if self.__check_downloader_instance(instance=service_info.instance):
            attrs['refresh'] = self.__get_config_item('dashboard_speed_widget_refresh')

        # 页面元素
        elements = self.__get_dashboard_speed_widget_elememts(service_info=service_info)
    ...
    def __fix_config(self, config: dict) -> dict:
        ...
        config_keys = config_copy.keys()
        if 'dashboard_widget_size' in config_keys:
            dashboard_widget_size = config_copy.get('dashboard_widget_size')
            config_copy['dashboard_widget_size'] = int(dashboard_widget_size) if dashboard_widget_size else None
        if 'dashboard_widget_refresh' in config_keys:
            dashboard_widget_refresh = config_copy.get('dashboard_widget_refresh')
            config_copy['dashboard_widget_refresh'] = int(dashboard_widget_refresh) if dashboard_widget_refresh else None
        if 'dashboard_widget_display_fields' in config_keys:
            dashboard_widget_display_fields = config_copy.get('dashboard_widget_display_fields')
            config_copy['dashboard_widget_display_fields'] = [field for field in dashboard_widget_display_fields if TorrentFieldMap.get(field)] if dashboard_widget_display_fields else []
    ...
    def __check_enable_downloader_sub_task(self, downloader_name: str) -> bool:
        """
        判断是否启用了下载器子任务
        ...
        if not downloader_name:
            return False
        return True if self.__get_config_item(config_key=f'{downloader_name}_enable_seeding') \
                       or self.__get_config_item(config_key=f'{downloader_name}_enable_tagging') \
                       or self.__get_config_item(config_key=f'{downloader_name}_enable_delete') else False

    def __check_enable_downloader_task(self, downloader_name: str) -> bool:
        """
        判断是否启用了下载器任务
        :param downloader_name: 下载器名称
        :return: 是否启用了下载器任务
        ...
        if not downloader_name:
            return False
        return True if self.__get_config_item(config_key=f'{downloader_name}_enable') \
                       and self.__check_enable_downloader_sub_task(downloader_name=downloader_name) else False
    ...
    def __check_enable_any_task(self) -> bool:
        ...
        :return: 是否启用了任意任务
        """
        downloader_configs = self.__get_downloader_configs()
        if not downloader_configs:
            return False
        for downloader_name in downloader_configs.keys():
            if self.__check_enable_downloader_task(downloader_name=downloader_name):
                return True
        return False
    ...
    def __check_need_delete_for_qbittorrent(self, torrent: TorrentDictionary, context: TaskContext) -> Tuple[bool, str]:
        ...
        if download_file_deleted_event_data:
            ...
            match, torrent_data_path = self.__check_torrent_match_file_for_qbittorrent(torrent=torrent,
            ...
            if not os.path.exists(torrent_data_path):
                return True, "源文件删除事件", True
            # 否则根据【事件删种策略】处理删种
            else:
                event_delete_torrent_strategy = self.__get_config_item(config_key="event_delete_torrent_strategy")
                # 如果是【提前删种】就返回删种，但不删数据文件
                if EventDeleteTorrentStrategy.EARLY.name == event_delete_torrent_strategy:
                    return True, f"源文件删除事件[{EventDeleteTorrentStrategy.EARLY.name_}]", False
        # 下载任务删除事件触发
        elif download_deleted_event_data:
            torrent_info = download_deleted_event_data
            match = self.__check_torrent_match_torrent_info(torrent_hash=torrent.get('hash'),
                                                            torrent_data_file_name=torrent.get('name'),
                                                            torrent_size=torrent.get('total_size'),
                                                            torrent_info=torrent_info)
            if match:
                return True, "下载任务删除事件", True
        return False, None, None

    def __check_need_delete_for_transmission(self, torrent: Torrent, context: TaskContext) -> Tuple[bool, str]:
        """
        检查tr种子是否满足删除条件
        :param context: 任务上下文
        :return: 是否删种, 删种原因, 是否删除文件
        ...
        if download_file_deleted_event_data:
            # 根据伴随的源文件删除事件判断是否应该删种：如果当前种子和事件匹配并且种子中已经不存在数据文件时就需要删除
            match, torrent_data_path = self.__check_torrent_match_file_for_transmission(torrent=torrent,
                                                                                        source_file_info=download_file_deleted_event_data)
            if not match:
                return False, None, None
            # 如果匹配的种子数据路径不存在，说明数据文件已经（全部）被删除了，那么就允许删种
        ...
        # 下载任务删除事件触发
            ...
            torrent_info = download_deleted_event_data
            match = self.__check_torrent_match_torrent_info(torrent_hash=torrent.hashString,
    ...
    def __check_torrent_match_file_for_qbittorrent(self, torrent: TorrentDictionary,
        ...
        if not torrent or not source_file_info:
            return False, None
        return self.__check_torrent_match_file(torrent_hash=torrent.get('hash'),
                                               torrent_data_file_name=torrent.get('name'),
                                               source_hash=None,
                                               source_file_path=source_file_info.get('src'))

    def __check_torrent_match_file_for_transmission(self, torrent: Torrent,
                                                    source_file_info: dict) -> Tuple[bool, Optional[str]]:
        """
        检查种子和源文件是否匹配
        :param torrent: 种子
        ...
        if not torrent or not source_file_info:
            return False, None
        return self.__check_torrent_match_file(torrent_hash=torrent.hashString,
                                               torrent_data_file_name=torrent.get('name'),
                                               source_hash=None,
                                               source_file_path=source_file_info.get('src'))

    def __check_torrent_match_file(self, torrent_hash: str,
    ...
    def __check_torrent_match_torrent_info(self,
                                           torrent_hash: str,
                                           torrent_data_file_name: str,
                                           torrent_size: int,
    ...
    def __run_for_qbittorrent(self, service_info: ServiceInfo, context: TaskContext = None) -> TaskContext:
        """
        针对qb下载器运行插件任务
    ...
    def __seeding_batch_for_qbittorrent(self, downloader_name: str, torrents: List[TorrentDictionary]) -> int:
        """
        qb批量自动做种
        :return: 做种数
        """
        logger.info(f'下载器[{downloader_name}] - 批量自动做种开始...')
        count = 0
        if not torrents:
            return count
        for torrent in torrents:
            if self.__exit_event.is_set():
                ...
                return count
            if self.__seeding_single_for_qbittorrent(downloader_name=downloader_name, torrent=torrent):
                count += 1
        logger.info(f'下载器[{downloader_name}] - 批量自动做种结束')
        return count

    def __seeding_single_for_qbittorrent(self, downloader_name: str, torrent: TorrentDictionary) -> bool:
        """
        qb单个自动做种
        :return: 是否执行
        """
        ...
        logger.info(f"下载器[{downloader_name}] - 单个自动做种完成: hash = {hash_str}, name = {name}, size = {total_size}")
        return True
    ...
    def __tagging_batch_for_qbittorrent(self,
        ...
        :return: 打标数
        """
        logger.info(f'下载器[{downloader_name}] - 批量自动标签开始...')
        count = 0
        if not torrents:
            return count
        for torrent in torrents:
            if self.__exit_event.is_set():
                logger.warn('插件服务正在退出，子任务终止')
                return count
            if self.__tagging_single_for_qbittorrent(downloader_name=downloader_name, qbittorrent=qbittorrent, torrent=torrent):
                count += 1
        logger.info(f'下载器[{downloader_name}] - 批量自动标签结束')
        return count
    ...
    def __tagging_single_for_qbittorrent(self,
        ...
        logger.info(f"下载器[{downloader_name}] - 单个自动标签成功: hash = {hash_str}, name = {name}, size = {total_size}, before = {tags}, after = {after}")
        return True
    ...
    def __delete_batch_for_qbittorrent(self, downloader_name: str, qbittorrent: Qbittorrent, torrents: List[TorrentDictionary], context: TaskContext) -> int:
        """
        qb批量自动删种
        :return: 删种数
        """
        logger.info(f'下载器[{downloader_name}] - 批量自动删种开始...')
        count = 0
        if not torrents:
            return count
        # 要从列表中移除的种子
    ...
    def __delete_single_for_qbittorrent(self, downloader_name: str, qbittorrent: Qbittorrent, torrent: TorrentDictionary, context: TaskContext) -> bool:
        ...
        name = self.__extract_torrent_value_for_qbittorrent(torrent=torrent, field=TorrentField.NAME)
        total_size = self.__extract_torrent_value_for_qbittorrent(torrent=torrent, field=TorrentField.TOTAL_SIZE)
        logger.info(f"下载器[{downloader_name}] - 单个自动删种完成: hash = {hash_str}, name = {name}, size = {total_size}, reason = {reason}")
        return True

    def __run_for_transmission(self, service_info: ServiceInfo, context: TaskContext = None) -> TaskContext:
        """
        针对tr下载器运行插件任务
        :param service_info: 下载器服务信息
        :param context: 任务上下文
        ...
        if not self.__check_downloader_service(service_info=service_info):
            return context
        if service_info.type != "transmission" or not isinstance(service_info.instance, Transmission):
            return context
        transmission: Transmission = service_info.instance
        ...
        try:
            ...
            # 自动标签
            if enable_tagging:
                result.set_tagging(self.__tagging_batch_for_transmission(downloader_name=downloader_name, transmission=transmission, torrents=torrents))
                if self.__exit_event.is_set():
                    logger.warn(f'插件服务正在退出，任务终止[{downloader_name}]')
                    return context
            # 自动做种
            if enable_seeding:
                result.set_seeding(self.__seeding_batch_for_transmission(downloader_name=downloader_name, transmission=transmission, torrents=torrents))
                if self.__exit_event.is_set():
                    logger.warn(f'插件服务正在退出，任务终止[{downloader_name}]')
                    return context
            # 自动删种
            if enable_delete:
                result.set_delete(self.__delete_batch_for_transmission(downloader_name=downloader_name, transmission=transmission, torrents=torrents, context=context))
                if self.__exit_event.is_set():
                    logger.warn(f'插件服务正在退出，任务终止[{downloader_name}]')
                    return context
    ...
    def __seeding_batch_for_transmission(self, downloader_name: str, transmission: Transmission, torrents: List[Torrent]) -> int:
        """
        tr批量自动做种
        :return: 做种数
        """
        ...
        for torrent in torrents:
            if self.__exit_event.is_set():
                logger.warn('插件服务正在退出，子任务终止')
                return count
            if self.__seeding_single_for_transmission(downloader_name=downloader_name, transmission=transmission, torrent=torrent):
                count += 1
        logger.info(f'下载器[{downloader_name}] - 批量自动做种结束')
        return count

    def __seeding_single_for_transmission(self, downloader_name: str, transmission: Transmission, torrent: Torrent) -> bool:
        """
        tr单个自动做种
        :return: 是否执行
        """
    ...
    def __tagging_batch_for_transmission(self, downloader_name: str, transmission: Transmission, torrents: List[Torrent]) -> int:
        """
        tr批量自动标签
        :return: 打标数
        """
        ...
        for torrent in torrents:
            if self.__exit_event.is_set():
                logger.warn('插件服务正在退出，子任务终止')
                return count
            if self.__tagging_single_for_transmission(downloader_name=downloader_name, transmission=transmission, torrent=torrent):
                count += 1
        logger.info(f'下载器[{downloader_name}] - 批量自动标签结束')
        return count

    def __tagging_single_for_transmission(self, downloader_name: str, transmission: Transmission, torrent: Torrent) -> bool:
        """
        tr单个自动标签
        :return: 是否执行
        """
        ...
        logger.info(f"下载器[{downloader_name}] - 单个自动标签成功: hash = {hash_str}, name = {name}, size = {total_size}, before = {before}, after = {after}")
        # Flush 标签
    ...
    def __delete_batch_for_transmission(self, downloader_name: str, transmission: Transmission, torrents: List[Torrent], context: TaskContext) -> int:
        """
        tr批量自动删种
        :return: 删种数
        """
        ...
        for torrent in torrents:
            if self.__exit_event.is_set():
                logger.warn('插件服务正在退出，子任务终止')
                return count
            if (self.__delete_single_for_transmission(downloader_name=downloader_name, transmission=transmission, torrent=torrent, context=context)):
                count += 1
                torrents_delete.append(torrent)
        if torrents_delete:
            for torrent in torrents_delete:
    ...
    def __delete_single_for_transmission(self, downloader_name: str, transmission: Transmission, torrent: Torrent, context: TaskContext) -> bool:
        """
        tr单个自动删种
        :return: 是否执行
        """
        ...
        if self.__exists_exclude_tag(torrent_tags):
            return False
        need_delete, reason, delete_file = self.__check_need_delete_for_transmission(torrent=torrent, context=context)
        if not need_delete:
            return False
        transmission.delete_torrents(delete_file=delete_file, ids=hash_str)
        # 日志
        name = self.__extract_torrent_value_for_transmission(torrent=torrent, field=TorrentField.NAME)
        total_size = self.__extract_torrent_value_for_transmission(torrent=torrent, field=TorrentField.TOTAL_SIZE)
        logger.info(f"下载器[{downloader_name}] - 单个自动删种完成: hash = {hash_str}, name = {name}, size = {total_size}, reason = {reason}")
    ...
    def __build_dashboard_widget_torrent_table_body_content(self,
        ...
        if data:
            ...
            empty_text = '暂无数据' if service_info and self.__check_downloader_instance(instance=service_info.instance) else '目标下载器配置无效'
            return [{
                'component': 'tr',
                'props': {
                    'class': 'text-sm'
    ...
    def __build_dashboard_widget_torrent_table_body(self,
                                                    service_info: ServiceInfo,
                                                    data: List[List[Any]],
                                                    field_count: int) -> dict:
        """
        构造仪表板组件种子表体
        """
        return {
            'component': 'tbody',
            'content': self.__build_dashboard_widget_torrent_table_body_content(service_info=service_info, data=data, field_count=field_count)
        }

    def __get_dashboard_widget_target_downloader_names(self, config_key: str) -> List[str]:
        """
        获取仪表板组件目标下载器names
        """
        target_downloader_names = []
        if not config_key:
            return target_downloader_names
        target_downloaders = self.__get_config_item(config_key)
        if not target_downloaders:
            return target_downloader_names
        default_downloader_name = self.__get_default_downloader_name()
        for target_downloader in target_downloaders:
            if target_downloader == self.__default_value_standing:
                target_downloader = default_downloader_name
            if target_downloader and target_downloader not in target_downloader_names:
                target_downloader_names.append(target_downloader)
        return target_downloader_names

    def __get_dashboard_active_torrent_widget_target_downloader_names(self) -> List[str]:
        """
        获取仪表板活动种子组件目标下载器names
        """
        return self.__get_dashboard_widget_target_downloader_names(config_key='dashboard_widget_target_downloaders')

    def __get_dashboard_speed_widget_target_downloader_names(self) -> List[str]:
        """
        获取仪表板实时速率组件目标下载器names
        """
        return self.__get_dashboard_widget_target_downloader_names(config_key='dashboard_speed_widget_target_downloaders')
    ...
    def __get_downloader_active_torrent_data(self,
        ...
        if not fields:
            fields = self.__get_dashboard_active_torrent_widget_display_fields()
        if isinstance(service_info.instance, Qbittorrent):
            return self.__get_qbittorrent_active_torrent_data(qbittorrent=service_info.instance, fields=fields)
        elif isinstance(service_info.instance, Transmission):
            return self.__get_transmission_active_torrent_data(transmission=service_info.instance, fields=fields)
        else:
            return None
    ...
    def __convert_qbittorrent_torrents_data(self,
    ...
    def __get_dashboard_active_torrent_widget_elememts(self, service_info: ServiceInfo) -> list:
        """
        获取仪表板活动种子组件元素
        """
        if not service_info:
        ...
        data = self.__get_downloader_active_torrent_data(service_info=service_info, fields=fields)
        if self.__exit_event.is_set():
            logger.warn('插件服务正在退出，操作取消')
            return None
        return [{
            ...
            'content': [
                ...
                self.__build_dashboard_widget_torrent_table_body(service_info=service_info, data=data, field_count=field_count)
            ]
        }]

    def __get_downloader_transfer_info(self, service_info: ServiceInfo) -> DownloaderTransferInfo:
        """
        获取下载器传输信息
        """
        if not self.__check_downloader_service(service_info=service_info) or not self.__check_downloader_instance(instance=service_info.instance):
            return DownloaderTransferInfo()
        if isinstance(service_info.instance, Qbittorrent):
            return self.__get_qbittorrent_transfer_info(qbittorrent=service_info.instance, downloader_name=service_info.name)
        elif isinstance(service_info.instance, Transmission):
            return self.__get_transmission_transfer_info(transmission=service_info.instance, downloader_name=service_info.name)
        return DownloaderTransferInfo()

    def __get_qbittorrent_transfer_info(self, qbittorrent: Qbittorrent, downloader_name: str) -> DownloaderTransferInfo:
        """
        获取qb下载器传输信息
        """
        ...
        maindata = self.__get_qbittorrent_maindata(qbittorrent=qbittorrent, downloader_name=downloader_name)
        if maindata:
    ...
    def __get_qbittorrent_maindata(self, qbittorrent: Qbittorrent, downloader_name: str):
        """
        获取qb的maindata
        """
        cache_key = f"qbittorrent_maindata_{downloader_name}"
        maindata = self.__ttl_cache.get(cache_key)
        if not maindata and qbittorrent and qbittorrent.qbc:
            maindata = qbittorrent.qbc.sync_maindata()
            self.__ttl_cache[cache_key] = maindata
        return maindata

    def __get_transmission_transfer_info(self, transmission: Transmission, downloader_name: str) -> DownloaderTransferInfo:
        """
        获取qb下载器传输信息
        """
        result = DownloaderTransferInfo()
        ...
        session = self.__get_transmission_session(transmission=transmission, downloader_name=downloader_name)
        if session:
            result.free_space = StringUtils.str_filesize(session.download_dir_free_space)
        return result

    def __get_transmission_session(self, transmission: Transmission, downloader_name: str):
        """
        获取tr的session
        """
        cache_key = f"transmission_session_{downloader_name}"
        session = self.__ttl_cache.get(cache_key)
        if not session and transmission and transmission.trc:
    ...
    def __get_dashboard_speed_widget_elememts(self, service_info: ServiceInfo) -> list:
        """
        获取仪表板实时速率组件元素
        """
        if not service_info:
        ...
        if self.__exit_event.is_set():
            logger.warn('插件服务正在退出，操作取消')
            return None
        data = self.__get_downloader_transfer_info(service_info=service_info)
        if self.__exit_event.is_set():
            logger.warn('插件服务正在退出，操作取消')
            return None
        list_items = [
    ...
    def __get_downloader_configs(self, include_disabled: bool = False) -> Dict[str, DownloaderConf]:
        """
        获取全部下载器配置
        """
        downloader_configs: Dict[str, DownloaderConf] = self.__downloader_helper.get_configs(include_disabled=include_disabled)
        return downloader_configs or {}

    def __get_default_downloader_config(self) -> DownloaderConf:
        """
        获取默认下载器配置
        """
        downloader_configs: Dict[str, DownloaderConf] = self.__get_downloader_configs(include_disabled=True)
        if downloader_configs:
            for _, downloader_config in downloader_configs.items():
                if downloader_config and downloader_config.default:
                    return downloader_config
        return None

    def __get_default_downloader_name(self):
        """
        获取默认下载器名称
        """
        default_downloader_config: DownloaderConf = self.__get_default_downloader_config()
        return default_downloader_config.name if default_downloader_config else None

    @classmethod
    def __check_downloader_config(cls, downloader_config: DownloaderConf) -> bool:
        """
        检查下载器配置
        """
        return True if downloader_config and downloader_config.name and downloader_config.type and downloader_config.config and downloader_config.enabled else False

    @classmethod
    def __check_downloader_service(cls, service_info: ServiceInfo) -> bool:
        """
        检查下载器服务信息
        """
        if not service_info or not service_info.name or not service_info.instance or not service_info.module or not service_info.type or not service_info.config:
            return False
        downloader_config: DownloaderConf = service_info.config
        return cls.__check_downloader_config(downloader_config=downloader_config)

    def __get_downloader_services(self, check: bool = True) -> Dict[str, ServiceInfo]:
        """
        获取所有下载器服务信息
        """
        downloader_services = self.__downloader_helper.get_services() or {}
        if check:
            downloader_services = {
                downloader_name: downloader_service
                for downloader_name, downloader_service in downloader_services.items() if self.__check_downloader_service(service_info=downloader_service)
            }
        return downloader_services

    def __get_downloader_service(self, name: str, check: bool = True) -> Optional[ServiceInfo]:
        """
        获取指定下载器服务信息
        """
        downloader_services = self.__get_downloader_services(check=check)
        return downloader_services.get(name) if downloader_services else None

    @classmethod
    def __check_downloader_instance(cls, instance: Union[Qbittorrent, Transmission]) -> bool:
        """
        检查下载器实例
        """
        if not instance:
            return False
        if isinstance(instance, Qbittorrent):
            return True if instance.qbc else False
        if isinstance(instance, Transmission):
            return True if instance.trc else False
        return False
