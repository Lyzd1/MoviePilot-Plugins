...
class TrafficAssistant(_PluginBase):
    ...
    def __update_brush_sites(self, site_id: int, enable: bool, plugin_id: str) -> [bool, str]:
        ...
        # 新版 BrushFlow：tasks 任务列表（每个站点独立任务）
        ...
        if isinstance(tasks, list):
            ...
            if not target_task:
                action_msg = "刷流站点：未配置刷流任务，请先在刷流插件中配置"
                logger.warning(action_msg)
                return False, action_msg

            task_enabled = bool(target_task.get("enabled", True))
            if enable == task_enabled:
                action_msg = "刷流站点：无需调整"
                logger.info(action_msg)
                return False, action_msg

            target_task["enabled"] = enable
            config_needs_update = True
            action_msg = "刷流站点：任务已暂停" if not enable else "刷流站点：任务已启用"
            logger.info(action_msg)
