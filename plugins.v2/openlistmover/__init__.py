...
class OpenlistMover(_PluginBase):
    ...
    _dir_cache_seconds = 3600  # 目录列表缓存时长（秒），默认 1 小时；0 或负值 = 不缓存
    _last_dir_cache_sweep = 0.0  # 上次惰性清扫时间戳
    _dir_cache_sweep_interval = 600  # 惰性清扫间隔（秒），最多每 10 分钟扫一次
    # ==========================================================
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            ...
                                            "variant": "tonal",
                                            "title": "洗版模式配置",
                                            "text": "开启后，探测到新媒体文件时（执行移动前）会提前 list 一次目标目录并缓存（默认 1 小时，可配置，同目录并发只发一次请求）：检查是否存在同名的旧视频/音频文件——同名同后缀（重投旧文件）与跨后缀变体（洗版替换）都会在移动前直接删除（仅视频/音频文件触发该预检查，封面/nfo 等额外后缀文件不会触发，避免误删）。缓存保存在内存中，由插件增量维护：移动成功的新文件追加进缓存、洗版删除同步移除，缓存有效期内同一目录后续洗版可直接命中旧版本并替换；空目录会缓存为空列表，首个文件移动成功后自动追加。条目采用滑动续期：有效期内每次命中/更新都会刷新寿命，目录持续活跃则缓存持续有效，空闲超过 TTL 才过期；过期即删除（读取/追加/删除时发现过期立即移除，并每 10 分钟自动清扫一次过期条目与单飞锁，回收内存）；设为 0 = 不缓存（每次重新拉取）。外部变化在缓存过期后重新拉取；若移动时仍遇到目标文件已存在 (403 exists)（如缓存未覆盖的外部新增文件等极端场景），将自动使用覆盖模式 (overwrite: true) 作为最后防线重新移动。洗版成功后，会先删除旧的 STRM 文件，等待指定延迟后再重新生成。",
                                        },
                                    }
                                ]
                            }
    ...
    def _check_and_clean_similar_files(self, dst_dir: str, target_file: str) -> bool:
        """
        检查目标目录中是否存在旧版本文件（同名同后缀或跨后缀变体），如果存在则删除
        ...
        # 本地比对：查找相同文件名（不含后缀）的旧版本条目。
        # 同后缀（重投同名文件）与跨后缀变体（洗版替换）统一在移动前删除；
        # 缓存未覆盖的外部新增/并发等极端场景由 403 exists 传统洗版兜底覆盖。
    ...
    def _get_dir_listing_cached(self, dst_dir: str) -> Optional[List[str]]:
        ...
        # 0. 周期性惰性清扫：删除所有过期条目与孤儿锁，回收内存（无后台线程，最多每 10 分钟一次）
        if time.time() - self._last_dir_cache_sweep >= self._dir_cache_sweep_interval:
            self._last_dir_cache_sweep = time.time()
            self._sweep_dir_cache()
        ...
        with self._dir_cache_lock:
            ...
            if cached:
                ...
                logger.debug(f"洗版模式：目录列表缓存 {key} 已过期，删除条目并重新拉取")
        ...
        with key_lock:
            ...
            # 仅当开启缓存（时长 > 0）时才存储；0/负值 = 不缓存，每次重新拉取
            if listing is not None and self._dir_cache_seconds > 0:
                with self._dir_cache_lock:
        ...
        with self._dir_cache_lock:
            ...
            if cached:
                ...
                if self._dir_cache_seconds > 0 and now - cache_time < self._dir_cache_seconds:
                    ...
                    self._dir_listing_cache.pop(key, None)
                    logger.debug(f"洗版模式：目录列表缓存 {key} 已过期，删除条目（不追加 {name}）")
            else:
        ...
        with self._dir_cache_lock:
            ...
            if cached:
                ...
                if self._dir_cache_seconds > 0 and now - cache_time < self._dir_cache_seconds:
                    ...
                    self._dir_listing_cache.pop(key, None)
                    logger.debug(f"洗版模式：目录列表缓存 {key} 已过期，删除条目")
